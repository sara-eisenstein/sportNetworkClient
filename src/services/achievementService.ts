...
export const addAchievement = async (achievement: Achievement): Promise<string> => {
  try {
    const response = await axios.post(`${API_URL}/api/Achievement`, achievement);
...
export const getAchievementsByUserId = async (userId: number): Promise<Achievement[]> => {
  try {
    ...
    return response.data.map((ach) => ({
      ...ach,
      dateEarned: new Date(ach.dateEarned), // ממיר תאריך לפורמט JS Date
    }));
  } catch (error) {
